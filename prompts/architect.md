...
- No external asset files (textures, sounds, models, fonts hosted locally). Visuals use Babylon procedural builders, procedural/standard/pbr materials, colors, glow, particles, or data URIs only.
- Audio, if any, uses the Web Audio API oscillators / noise. No mp3/wav fetches.
- Must boot from file://, from a local static server, and from a GitHub Pages project URL (including subdirectory paths). Never assume the site is hosted at domain root.
...
1. Game fantasy and win/lose rules
